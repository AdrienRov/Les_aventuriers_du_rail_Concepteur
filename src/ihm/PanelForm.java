...
public class PanelForm extends JPanel implements ActionListener, CellEditorListener {
    ...
    public void initPanel(int numPanel) {
        ...
        if (numPanel == 2) {
            ...
            JLabel label = new JLabel(
                    "<html><center>Cliquer sur <br> deux nœuds pour <br> créer une arête</center></html>");
            label.setForeground(Color.WHITE);
            label.setFont(new Font("Arial", Font.BOLD, 20));
            this.add(label, g);
            g.gridy = g.gridy + 1;
    ...
    public void refreshTabTrajet() {
        ...
        for (int i = 0; i < this.ctrl.getAllTrajets().size(); i++) {
            ...
            System.out.println("VALLLLLLL = " + a.getNoeudDepart().getNom() + " " + a.getNbVoiture() + " "
                    + a.getNoeudarrive().getNom());
    ...
    public void editingStopped(ChangeEvent e) {
        ...
        if (this.etat == 3) {
            ...
            if (this.table.getSelectedColumn() == 2) {
                ...
                try {
                    score = Integer.parseInt(this.table
                            .getValueAt(this.table.getSelectedRow(), this.table.getSelectedColumn()).toString());
                    System.out.println("colonnes = " + this.table.getSelectedColumn());
